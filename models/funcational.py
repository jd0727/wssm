...
def xywhuvsT_align(feats, ids_b, xywhuvs, size, spatial_scale=1.0, kernel_size=2):
    ...
    kernel_offset = _create_mesh(kernel_size, kernel_size, feats.device) - kernel_size / 2
    ...
    meshes = (_create_mesh(Hr, Wr, device=feats.device) / size - 0.5) * whs
...
def xywhuvsT_place(feats, ids_b, xywhuvs, size, spatial_scale=1.0, kernel_size=2):
    ...
    meshes_proj = torch.bmm(_create_mesh(Hr, Wr, device=feats.device) - xys, mats) / whs
...
class RProj(Function):
    ...
    def forward(ctx, feats, alphas, background):
        ...
        fliter_hit = val_proj > thres
        val_proj[~fliter_hit] = background
...
class MILR(Function):
    ...
    def proj_v(Nb, Hf, Wf, L, idx_ori, idx_proj, feats_p, thres, background, lmins, lmaxs, with_log):
        ...
        filter_pos = filter_l * fliter_hit
...
# <editor-fold desc='色域变换'>
RGB2XYZ = torch.Tensor([[0.412453, 0.357580, 0.180423],
                        [0.212671, 0.715160, 0.072169],
                        [0.019334, 0.119193, 0.950227]])
XYZ2RGB = torch.Tensor([[3.240479, -1.537150, -0.498535],
                        [-0.969256, 1.875992, 0.041556],
                        [0.055648, -0.204043, 1.057311]])


def imgsT_rgb2xyz(imgsT: torch.Tensor) -> torch.Tensor:
    Nb, C, H, W = imgsT.size()
    mat = RGB2XYZ.to(imgsT.device)[None].expand(Nb, 3, 3)
    imgsT = torch.bmm(mat, imgsT.view(Nb, C, H * W)).view(Nb, C, H, W)
    return imgsT


def imgsT_xyz2rgb(imgsT: torch.Tensor) -> torch.Tensor:
    Nb, C, H, W = imgsT.size()
    mat = XYZ2RGB.to(imgsT.device)[None].expand(Nb, 3, 3)
    imgsT = torch.bmm(mat, imgsT.view(Nb, C, H * W)).view(Nb, C, H, W)
    return imgsT


def _unlinear_trans(value):
    THRES = (6 / 29) ** 3
    value = torch.where(value > THRES, torch.pow(value, 1 / 3), ((29 / 6) ** 2) / 2 * value + 16 / 116)
    return value


def imgsT_xyz2lab(imgsT: torch.Tensor) -> torch.Tensor:
    x, y, z = torch.chunk(imgsT, chunks=3, dim=1)
    x, y, z = _unlinear_trans(x / 0.950456), _unlinear_trans(y), _unlinear_trans(z / 1.088754)
    L = 116 * y - 16
    a = 500 * (x - y)
    b = 200 * (y - z)
    imgsT = torch.cat([L / 100, a / 128, b / 128], dim=1)
    return imgsT


def imgsT_rgb2lab(imgsT: torch.Tensor) -> torch.Tensor:
    return imgsT_xyz2lab(imgsT_rgb2xyz(imgsT))


# </editor-fold>

# <editor-fold desc='基于邻近相似度的损失函数'>
def nei_loss_with_img(imgs, feats, fliter_in, kernel_size=3, dilation=1, reduction='sum', theta=1.0, thres=2.0):
    ...
    # pows_sim = pows_sim >thres* fliter_in[:, None, :]
    fliter_in = fliter_in.view(Nb, 1, Hf * Wf)
    fliter_hit = fliter_in * (pows_sim > thres)
    if not torch.any(fliter_hit): return torch.as_tensor(0).to(feats.device)
    ...
    loss = torch.mean(kl_dist[fliter_hit])
...
def nei_loss_with_img2(imgs, feats, fliter_in, kernel_size=3, dilation=1, reduction='sum', theta=1.0, **kwargs):
    Nb, Cf, Hf, Wf = feats.size()
    N, C, H, W = imgs.size()
    if not (Hf == H and Wf == W and Nb == N):
        print(feats.size(), imgs.size())
    ...
    # pows_sim = pows_sim >thres* fliter_in[:, None, :]
    fliter_in = fliter_in.view(Nb, 1, Hf * Wf).expand(Nb, kernel_size * kernel_size - 1, Hf * Wf)
    thres = torch.sum(pows_sim * fliter_in, dim=(1, 2), keepdim=True) \
            / torch.sum(fliter_in, dim=(1, 2), keepdim=True).clamp_(min=1)
    fliter_hit = fliter_in * (pows_sim < thres)
    if not torch.any(fliter_hit): return torch.as_tensor(0).to(feats.device)

    mse_loss = torch.abs(cen_val - rnd_val)
    mse_loss = torch.sum(mse_loss, dim=1)
    loss = torch.mean(mse_loss[fliter_hit])
    return loss


def crf_loss_with_img(imgs, feats, fliter_in, kernel_size=3, dilation=1, sxy=0.5, srgb=0.1, **kwargs):
    Nb, Cf, Hf, Wf = feats.size()
    N, C, H, W = imgs.size()
    assert Hf == H and Wf == W and Nb == N
    pad_s = (kernel_size - 1) * dilation // 2
    kxs, kys = arange2d(kernel_size, kernel_size, feats.device)
    ...
    fliter_in = fliter_in.view(Nb, 1, Hf * Wf)
    if not torch.any(fliter_in): return torch.as_tensor(0).to(feats.device)
    pows = torch.exp(-(pows_sim / srgb) ** 2 - (kernel_offset / sxy) ** 2) * fliter_in

    loss = torch.sum(pows * pairs_l1) / torch.sum(pows)
    return loss


def crf_loss_with_img3(imgs, feats, fliter_in, kernel_size=3, dilation=1, simga=None, **kwargs):
    Nb, Cf, Hf, Wf = feats.size()
    N, C, H, W = imgs.size()
    assert Hf == H and Wf == W and Nb == N
    pad_s = (kernel_size - 1) * dilation // 2
    ...
    fliter_in = fliter_in.view(Nb, 1, Hf * Wf)
    if not torch.any(fliter_in): return torch.as_tensor(0).to(feats.device)
    sum_val = torch.sum(fliter_in, dim=(1, 2), keepdim=True).clamp_(min=1) * kernel_size * kernel_size
    if simga is None:
        simga = torch.sum(fliter_in * pows_sim, dim=(1, 2), keepdim=True) / sum_val.clamp_(min=1) + 1e-5

    pows = torch.exp(-(pows_sim / simga) ** 2) * fliter_in
...
def crf_loss_with_img4(imgs, feats, fliter_in, kernel_size=3, dilation=1, srgb=0.1, **kwargs):
    Nb, Cf, Hf, Wf = feats.size()
    N, C, H, W = imgs.size()
    assert Hf == H and Wf == W and Nb == N
    pad_s = (kernel_size - 1) * dilation // 2
    ...
    if not torch.any(fliter_in): return torch.as_tensor(0).to(feats.device)
    sum_val = torch.sum(fliter_in, dim=(1, 2), keepdim=True).clamp_(min=1) * kernel_size * kernel_size
    mean_val = torch.sum(fliter_in * pows_sim, dim=(1, 2), keepdim=True) / sum_val.clamp_(min=1) + 1e-5

    pows = torch.exp(-(pows_sim / mean_val) ** 2) * fliter_in
