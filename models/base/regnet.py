...
class BottleneckSE(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, neck_ratio=1, group_channels=1, has_se=False, act=ACT.RELU):
        ...
        self.conv1 = Ck1s1BA(in_channels=in_channels, out_channels=inner_channels, act=act)
        ...
        self.conv3 = Ck1s1BA(in_channels=inner_channels, out_channels=out_channels, act=None)

        self.act = ACT.build(act)
        if stride != 1 or in_channels != out_channels:
            self.shortcut = CpaBA(in_channels=in_channels, out_channels=out_channels,
...
class RegNetBkbn(nn.Module):
    def __init__(self, channelss, repeat_nums, group_channels=1, has_se=True, act=ACT.RELU):
        super(RegNetBkbn, self).__init__()
        self.pre = CpaBA(in_channels=3, out_channels=32, kernel_size=3, stride=2, act=act)
        self.stage1 = RegNetBkbn.BottleneckSERepeat(
            in_channels=32, out_channels=channelss[0], repeat_num=repeat_nums[0], stride=2,
            group_channels=group_channels, has_se=has_se, act=act)
...
class RegNetMain(RegNetBkbn):
    def __init__(self, channelss, repeat_nums, group_channels=1, has_se=True, act=ACT.RELU, num_cls=10):
        super(RegNetMain, self).__init__(channelss=channelss, repeat_nums=repeat_nums, group_channels=group_channels,
                                         has_se=has_se, act=act)
    ...
    @staticmethod
    def X002(act=ACT.RELU, num_cls=10):
        return RegNetMain(**RegNetBkbn.X002_PARA, act=act, num_cls=num_cls)

    @staticmethod
    def X004(act=ACT.RELU, num_cls=10):
        return RegNetMain(**RegNetBkbn.X004_PARA, act=act, num_cls=num_cls)

    @staticmethod
    def X006(act=ACT.RELU, num_cls=10):
        return RegNetMain(**RegNetBkbn.X006_PARA, act=act, num_cls=num_cls)

    @staticmethod
    def X008(act=ACT.RELU, num_cls=10):
        return RegNetMain(**RegNetBkbn.X008_PARA, act=act, num_cls=num_cls)
...
class RegNet(OneStageClassifier):
    ...
    @staticmethod
    def X002(device=None, pack=PACK.AUTO, num_cls=20, img_size=(512, 512)):
        backbone = RegNetMain.X002(act=ACT.RELU, num_cls=num_cls)
        return RegNet(backbone=backbone, device=device, pack=pack, img_size=img_size)

    @staticmethod
    def X004(device=None, pack=PACK.AUTO, num_cls=20, img_size=(512, 512)):
        backbone = RegNetMain.X004(act=ACT.RELU, num_cls=num_cls)
        return RegNet(backbone=backbone, device=device, pack=pack, img_size=img_size)

    @staticmethod
    def X006(device=None, pack=PACK.AUTO, num_cls=20, img_size=(512, 512)):
        backbone = RegNetMain.X006(act=ACT.RELU, num_cls=num_cls)
        return RegNet(backbone=backbone, device=device, pack=pack, img_size=img_size)

    @staticmethod
    def X008(device=None, pack=PACK.AUTO, num_cls=20, img_size=(512, 512)):
        backbone = RegNetMain.X008(act=ACT.RELU, num_cls=num_cls)
        return RegNet(backbone=backbone, device=device, pack=pack, img_size=img_size)


if __name__ == '__main__':
    model = RegNet.X006(img_size=(224, 224), num_cls=20, device=1)
