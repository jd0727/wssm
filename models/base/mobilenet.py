...
class InvertedBottleNeck(nn.Module):
    def __init__(self, in_channels, out_channels, inner_channels, kernel_size, stride, act=ACT.RELU6):
        ...
        self.stride = stride
        self.conv1 = Ck1s1BA(in_channels=in_channels, out_channels=inner_channels, bn=True, act=act)
        self.conv2 = CpadwBA(channels=inner_channels, stride=stride, kernel_size=kernel_size, bn=True, act=act)
        self.conv3 = Ck1s1BA(in_channels=inner_channels, out_channels=out_channels, bn=True, act=None)
        self.stride = stride
        if stride == 1:
...
class MobileNetV1Bkbn(nn.Module):
    def __init__(self, act=ACT.RELU, repeat_nums=(1, 1, 2, 2, 2),in_channels=3):
        super(MobileNetV1Bkbn, self).__init__()
        self.pre = CpaBA(in_channels=in_channels, out_channels=32, kernel_size=3, stride=2, act=act)
    ...
    @staticmethod
    def CdwRepeat(in_channels, out_channels, stride, repeat_num=1, act=ACT.RELU):
        backbone = []
        last_channels = in_channels
        for i in range(repeat_num):
            backbone.append(CpadwBA(channels=last_channels, kernel_size=3, stride=stride, bn=True, act=act))
            backbone.append(Ck1s1BA(in_channels=last_channels, out_channels=out_channels, bn=True, act=act))
...
class MobileNetV2Bkbn(nn.Module):
    def __init__(self, act=ACT.RELU6, repeat_nums=(1, 2, 3, 4, 3, 3, 1),in_channels=3):
        ...
        self.stage8 = Ck1BA(in_channels=320, out_channels=1280, act=act)
...
class MobileNetV3SmallBkbn(nn.Module):
    def __init__(self, act1=ACT.RELU, act2=ACT.HSILU,in_channels=3):
        ...
        self.pre = CpaBA(in_channels=in_channels, out_channels=16, kernel_size=3, stride=2, act=act1)
        ...
        self.stage4 = nn.Sequential(
            ...
            InvertedBottleNeckSE(96, 576, 96, kernel_size=5, stride=1, with_se=True, act=act2),
            Ck1BA(in_channels=96, out_channels=576, bn=True, act=act2),
        )
...
class MobileNetV3LargeBkbn(nn.Module):
    def __init__(self, act1=ACT.RELU, act2=ACT.HSILU,in_channels=3):
        ...
        self.pre = nn.Sequential(
            CpaBA(in_channels=in_channels, out_channels=16, kernel_size=3, stride=2, act=act1),
        ...
        self.stage4 = nn.Sequential(
            ...
            Ck1BA(in_channels=160, out_channels=960, bn=True, act=act2)
        )
