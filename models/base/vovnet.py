...
class SeModule(nn.Module):
    def __init__(self, channels, ratio=0.25):
        ...
        self.se_pth = nn.Sequential(
            ...
            Ck1BA(in_channels=channels, out_channels=inner_channels, bn=False, act=ACT.HSIG)
        )
...
class OSABlock(nn.Module):
    def __init__(self, in_channels, inner_channels, out_channels, conv_num, se=False, depthwise=False,
        ...
        self.trans_conv = Ck1s1BA(in_channels=in_channels, out_channels=inner_channels, act=act) if trans else None

        last_channels = inner_channels if trans else in_channels
        ConvType = Ck3k1dwBA if depthwise else Ck3BA
        convs = []
        ...
        self.concater = Ck1s1BA(in_channels=concat_channels, out_channels=out_channels, act=act)
...
class VoVNetBkbn(nn.Module):
    def __init__(self, pre_channelss, channelss, inner_channelss, conv_num, repeat_nums, se, depthwise, act=ACT.RELU):
        ...
        self.pre = nn.Sequential(
            Ck3BA(in_channels=3, out_channels=pre_channelss[0], stride=2, act=act),
