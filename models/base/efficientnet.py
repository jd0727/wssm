...
class EfficientNetBkbn(nn.Module):
    def __init__(self, ratio_depth, ratio_width, act=ACT.SILU, in_channels=3):
    ...
    @staticmethod
    def B2_CUS(act=ACT.SILU):
        return EfficientNetBkbn(**EfficientNetBkbn.B2_CUS_PARA, act=act)
...
class EfficientNetMain(EfficientNetBkbn):
    def __init__(self, ratio_depth, ratio_width, act=ACT.SILU, num_cls=0, in_channels=3):
        ...
        self.conv8 = Ck1BA(in_channels=in_channels, out_channels=inner_channels, act=act)
...
if __name__ == '__main__':
    model = EfficientNet.B3(device=1, num_cls=20, img_size=(224, 224))
