...
class UpSampleCat(nn.Module):
    def __init__(self, in_channels_s, in_channels_l, out_channels, repeat_num=1, num_feat_l=1,
                 act=ACT.RELU, with_convt=True):
        super().__init__()
        if with_convt:
            self.samper = CTk3BA(in_channels=in_channels_s, out_channels=out_channels, act=act, stride=2)
        ...
        for i in range(repeat_num):
            convs.append(Ck3s1BA(in_channels=in_channels, out_channels=out_channels, act=act))
...
class UNetV1Main(nn.Module):
    def __init__(self, channelss, num_cls=1, repeat_num=2, with_convt=True, act=ACT.RELU, img_size=(224, 224),
                 in_channels=3):
        ...
        for i in range(len(channelss)):
            self.convs.append(UNetV1Main.Ck3Repeat(
                in_channels=last_channels, out_channels=channelss[i], repeat_num=repeat_num, with_pool=i > 0, act=act))
        ...
        for i in range(len(channelss) - 1):
            self.uppers.append(UpSampleCat(
                ...
                act=act, with_convt=with_convt, num_feat_l=1, repeat_num=repeat_num))

        self.out = Ck1s1(in_channels=channelss[0], out_channels=num_cls)

    @staticmethod
    def Ck3Repeat(in_channels, out_channels, repeat_num=1, act=ACT.RELU, with_pool=True):
    ...
    @staticmethod
    def Std(img_size=(224, 224), num_cls=1, act=ACT.RELU):
        return UNetV1Main(**UNetV1Main.PARA_STD, img_size=img_size, num_cls=num_cls, act=act)

    @staticmethod
    def Tiny(img_size=(224, 224), num_cls=1, act=ACT.RELU):
        return UNetV1Main(**UNetV1Main.PARA_TINY, img_size=img_size, num_cls=num_cls, act=act)
...
class UNetV2Main(nn.Module):
    def __init__(self, channelss, num_cls=1, repeat_num=2, with_convt=True, act=ACT.RELU, img_size=(224, 224),
        ...
        self.upper02 = UpSampleCat(in_channels_s=channelss[1], in_channels_l=channelss[0], out_channels=channelss[0],
                                   act=act, with_convt=with_convt, num_feat_l=2, repeat_num=repeat_num)
        ...
        self.upper03 = UpSampleCat(in_channels_s=channelss[1], in_channels_l=channelss[0], out_channels=channelss[0],
                                   act=act, with_convt=with_convt, num_feat_l=3, repeat_num=repeat_num)
        ...
        self.upper04 = UpSampleCat(in_channels_s=channelss[1], in_channels_l=channelss[0], out_channels=channelss[0],
                                   act=act, with_convt=with_convt, num_feat_l=4, repeat_num=repeat_num)
    ...
    @staticmethod
    def Std(img_size=(224, 224), num_cls=1, act=ACT.RELU):
        return UNetV2Main(**UNetV2Main.PARA_STD, img_size=img_size, num_cls=num_cls, act=act)
...
class UNet(OneStageSegmentor):
    ...
    @staticmethod
    def V1Std(device=None, pack=None, num_cls=20, img_size=(224, 224)):
        backbone = UNetV1Main.Std(act=ACT.RELU, num_cls=num_cls + 1, img_size=img_size)
        return UNet(backbone=backbone, device=device, pack=pack, num_cls=num_cls, img_size=img_size)

    @staticmethod
    def V1Tiny(device=None, pack=None, num_cls=20, img_size=(224, 224)):
        backbone = UNetV1Main.Tiny(act=ACT.RELU, num_cls=num_cls + 1, img_size=img_size)
        return UNet(backbone=backbone, device=device, pack=pack, num_cls=num_cls, img_size=img_size)

    @staticmethod
    def V2Std(device=None, pack=None, num_cls=20, img_size=(224, 224)):
        backbone = UNetV2Main.Std(act=ACT.RELU, num_cls=num_cls + 1, img_size=img_size)
        return UNet(backbone=backbone, device=device, pack=pack, num_cls=num_cls, img_size=img_size)
