...
class ResUNetAResidual(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, dilations=(1, 3, 5), act=ACT.RELU):
        super(ResUNetAResidual, self).__init__()

        self.convs = nn.ModuleList()
        for dilation in dilations:
            self.convs.append(nn.Sequential(
                Ck3BA(in_channels=in_channels, out_channels=out_channels,
                      stride=stride, dilation=dilation, bn=True, act=act),
                Ck3BA(in_channels=out_channels, out_channels=out_channels,
...
class BottleneckFull(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, dilation=1, act=ACT.RELU):
        ...
        self.conv1 = Ck1s1BA(in_channels=in_channels, out_channels=out_channels,
                             bn=True, act=act)
        self.conv2 = Ck3BA(in_channels=out_channels, out_channels=out_channels,
                           stride=stride, dilation=dilation, bn=True, act=act)
        self.conv3 = Ck1s1BA(in_channels=out_channels, out_channels=out_channels,
                             bn=True, act=None)
...
class ResUNetEnc(nn.Module):
    def __init__(self, Module, repeat_nums, in_channels, out_channelss=(32, 64), act=ACT.RELU, strides=(2, 2)):
    ...
    def ModuleRepeat(Module, in_channels, out_channels, repeat_num=1, stride=1, act=ACT.RELU, with_pool=False,
        ...
        for i in range(repeat_num):
            backbone.append(Module(
                in_channels=last_channels, out_channels=out_channels, stride=stride, act=act, **kwargs))
...
class ResUNetAEnc(nn.Module):
    def __init__(self, Module, repeat_nums, in_channels, out_channelss=(32, 64), dilationss=((1, 3, 15), (1, 3, 15)),
        ...
        for i, (repeat_num, dilations) in enumerate(zip(repeat_nums, dilationss)):
            ...
            self.stages.append(ResUNetEnc.ModuleRepeat(
                Module, in_channels=in_channels, out_channels=out_channelss[i], stride=strides[i],
                repeat_num=repeat_num, act=act, dilations=dilations, with_pool=False))
...
class ResUNetADec(nn.Module):
    def __init__(self, Module, repeat_nums, in_channelss=(32, 64), out_channelss=(32, 64),
                 dilationss=((1, 3, 15), (1, 3, 15)), act=ACT.RELU, strides=(2, 2)):
...
class ResUNetMain(nn.Module):
    def __init__(self, Module, repeat_nums_enc, repeat_nums_dec, pre_channels,
        ...
        self.strides = strides
        self.pre = CpaBA(in_channels=in_channels, out_channels=pre_channels, kernel_size=7, stride=1, bn=True, act=act)
    ...
    @staticmethod
    def LV6(img_size=(224, 224), num_cls=20, act=ACT.RELU):
        return ResUNetMain(**ResUNetMain.PARA_LV6, img_size=img_size, num_cls=num_cls, act=act)

    @staticmethod
    def LV5(img_size=(224, 224), num_cls=20, act=ACT.RELU):
        return ResUNetMain(**ResUNetMain.PARA_LV5, img_size=img_size, num_cls=num_cls, act=act)

    @staticmethod
    def R50(img_size=(224, 224), num_cls=20, act=ACT.RELU):
        return ResUNetMain(**ResUNetMain.PARA_R50, img_size=img_size, num_cls=num_cls, act=act)

    @staticmethod
    def R34(img_size=(224, 224), num_cls=20, act=ACT.RELU):
        return ResUNetMain(**ResUNetMain.PARA_R34, img_size=img_size, num_cls=num_cls, act=act)


class ResUNetAMain(nn.Module):
    def __init__(self, Module, repeat_nums_enc, repeat_nums_dec, strides, pre_channels,
                 channelss, dilationss, num_cls=20, img_size=(224, 224), act=ACT.RELU, in_channels=3):
        super(ResUNetAMain, self).__init__()
        self.img_size = img_size
        self.num_cls = num_cls
        self.strides = strides
        self.pre = CpaBA(in_channels=in_channels, out_channels=pre_channels, kernel_size=5, stride=1, bn=True, act=act)
        self.enc = ResUNetAEnc(Module, repeat_nums_enc, in_channels=pre_channels, out_channelss=channelss,
                               dilationss=dilationss, act=act, strides=strides)
        self.psp1 = PSP(in_channels=channelss[-1], out_channels=channelss[-1], strides=(1, 2, 4, 8), act=act)
        self.dec = ResUNetADec(Module, repeat_nums_dec, in_channelss=channelss, out_channelss=channelss,
                               dilationss=dilationss, act=act, strides=strides)
        self.psp2 = PSP(in_channels=channelss[0], out_channels=channelss[0], strides=(1, 2, 4, 8), act=act)
        self.out = Ck1s1(in_channels=channelss[0] + pre_channels, out_channels=num_cls)
    ...
    @staticmethod
    def LV6(img_size=(224, 224), num_cls=20, act=ACT.RELU):
        return ResUNetAMain(**ResUNetAMain.PARA_LV6, img_size=img_size, num_cls=num_cls, act=act)

    @staticmethod
    def LV5(img_size=(224, 224), num_cls=20, act=ACT.RELU):
        return ResUNetAMain(**ResUNetAMain.PARA_LV5, img_size=img_size, num_cls=num_cls, act=act)


class ResUNet(OneStageSegmentor):
    def __init__(self, backbone, device=None, pack=None, num_cls=20, img_size=(224, 224)):
        super().__init__(backbone, device=device, pack=pack, num_cls=num_cls, img_size=img_size)

    @staticmethod
    def LV6(device=None, pack=None, num_cls=20, img_size=(224, 224)):
        backbone = ResUNetMain.LV6(act=ACT.RELU, num_cls=num_cls + 1, img_size=img_size)
        return ResUNet(backbone=backbone, device=device, pack=pack, num_cls=num_cls, img_size=img_size)

    @staticmethod
    def LV5(device=None, pack=None, num_cls=20, img_size=(224, 224)):
        backbone = ResUNetMain.LV5(act=ACT.RELU, num_cls=num_cls + 1, img_size=img_size)
        return ResUNet(backbone=backbone, device=device, pack=pack, num_cls=num_cls, img_size=img_size)

    @staticmethod
    def ALV6(device=None, pack=None, num_cls=20, img_size=(224, 224)):
        backbone = ResUNetAMain.LV6(act=ACT.RELU, num_cls=num_cls + 1, img_size=img_size)
        return ResUNet(backbone=backbone, device=device, pack=pack, num_cls=num_cls, img_size=img_size)

    @staticmethod
    def ALV5(device=None, pack=None, num_cls=20, img_size=(224, 224)):
        backbone = ResUNetAMain.LV5(act=ACT.RELU, num_cls=num_cls + 1, img_size=img_size)
        return ResUNet(backbone=backbone, device=device, pack=pack, num_cls=num_cls, img_size=img_size)
