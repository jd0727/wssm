...
class VGGBkbn(nn.Module):
    def __init__(self, repeat_nums=(1, 1, 2, 2, 2), act=ACT.RELU):
        ...
        self.stage1 = VGGBkbn.Ck3Repeat(in_channels=3, out_channels=64,
                                        repeat_num=repeat_nums[0], act=act, with_pool=False)
        self.stage2 = VGGBkbn.Ck3Repeat(in_channels=64, out_channels=128,
                                        repeat_num=repeat_nums[1], act=act, with_pool=True)
        self.stage3 = VGGBkbn.Ck3Repeat(in_channels=128, out_channels=256,
                                        repeat_num=repeat_nums[2], act=act, with_pool=True)
        self.stage4 = VGGBkbn.Ck3Repeat(in_channels=256, out_channels=512,
                                        repeat_num=repeat_nums[3], act=act, with_pool=True)
        self.stage5 = VGGBkbn.Ck3Repeat(in_channels=512, out_channels=512,
                                        repeat_num=repeat_nums[4], act=act, with_pool=True)

    @staticmethod
    def Ck3Repeat(in_channels, out_channels, repeat_num=1, with_pool=False, act=ACT.RELU):
        backbone = nn.Sequential()
        if with_pool:
            backbone.add_module('pool', nn.MaxPool2d(kernel_size=2, stride=2, padding=0, ceil_mode=False))
        last_channels = in_channels
        for i in range(repeat_num):
            backbone.add_module(str(i), Ck3BA(in_channels=last_channels, out_channels=out_channels, stride=1,act=act))
