...
class IMScheduler(metaclass=ABCMeta):
    def __init__(self, total_epoch=10):
    ...
    @staticmethod
    def Const(img_size):
        return IMSConst(img_size=img_size, total_epoch=1)

    @staticmethod
    def Rand(min_size, max_size, devisor=32, keep_ratio=True, keep_epoch=1, max_first=True, max_last=True,
             total_epoch=1):
        return IMSRand(min_size=min_size, max_size=max_size, devisor=devisor, keep_ratio=keep_ratio,
                       keep_epoch=keep_epoch, max_first=max_first, max_last=max_last, total_epoch=total_epoch)

    @staticmethod
    def convert(imscheduler):
        if isinstance(imscheduler, IMScheduler):
            return imscheduler
        elif isinstance(imscheduler, tuple) or isinstance(imscheduler, list):
            return IMSConst(img_size=imscheduler, total_epoch=1)
        else:
            raise Exception('fmt err' + imscheduler.__class__.__name__)


class IMSConst(IMScheduler):
    def __init__(self, img_size, total_epoch=10):
        super().__init__(total_epoch=total_epoch)
...
class IMSRand(IMScheduler):
    def __init__(self, min_size, max_size, devisor=32, keep_ratio=True, keep_epoch=1, max_first=True, max_last=True,
                 total_epoch=10):
        super().__init__(total_epoch=total_epoch)
    ...
    def __getitem__(self, epoch_ind):
        if (self.max_first and epoch_ind <= 0) \
                or (self.max_last and epoch_ind >= self.total_epoch - self.keep_epoch):
            img_size = self.max_size
        elif self.kpd < self.keep_epoch:
            img_size = self.last_size
        else:
            img_size = self._rand_size()
            self.kpd = 0
        self.kpd = self.kpd + 1
...
if __name__ == '__main__':
    ims = IMSRand(max_size=(800, 800), min_size=(480, 480), devisor=32, keep_epoch=2, total_epoch=50)
    ls = [ims[i] for i in range(50)]
    print(ls)
...
class LRScheduler(metaclass=ABCMeta):
    ...
    @staticmethod
    def convert(lrscheduler):
        if isinstance(lrscheduler, LRScheduler):
            return lrscheduler
        elif isinstance(lrscheduler, float):
            return LRSConst(lr=lrscheduler, total_epoch=1, iter_per_epoch=1)
        else:
            raise Exception('fmt err' + lrscheduler.__class__.__name__)
...
#     from visual import *
#
#     lrscheduler = LRScheduler.WARM_COS(lr=0.0025, warm_epoch=1, total_epoch=300)
#     # lrscheduler = LRScheduler.WARM_STEP(lr=0.1, warm_epoch=50, milestones=(20, 30), gamma=0.1, total_epoch=100)
#     lrscheduler.iter_per_epoch = 100
#     plt.plot(lrscheduler.lr_list)
